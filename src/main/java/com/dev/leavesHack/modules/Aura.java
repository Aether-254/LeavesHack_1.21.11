    ...
    private void doAura() {
        ...
        if (autoSwitch.get() != SwitchMode.None && !itemInHand()) {
            Predicate<ItemStack> predicate = switch (weapon.get()) {
                ...
                case Sword -> stack -> stack.getItem().getDefaultStack().isIn(ItemTags.SWORDS);
                case Mace -> stack -> stack.getItem() instanceof MaceItem;
                case Trident -> stack -> stack.getItem() instanceof TridentItem;
                case All -> stack -> stack.getItem() instanceof AxeItem || stack.getItem().getDefaultStack().isIn(ItemTags.SWORDS) || stack.getItem() instanceof MaceItem || stack.getItem() instanceof TridentItem;
                default -> o -> true;
            };
        ...
        if (mc.player != null) {
            Objects.requireNonNull(mc.getNetworkHandler()).sendPacket(PlayerInteractEntityC2SPacket.attack(target, mc.player.isSneaking()));
        }
        mc.attackCooldown = 0;
        ...
        for (Entity entity : mc.world.getEntities()) {
            ...
            if (ignoreTamed.get()) {
                if (entity instanceof Tameable tameable
                        && tameable.getOwnerReference().getUuid() != null
                        && tameable.getOwnerReference().getUuid().equals(mc.player.getUuid())
                ) continue;
            }
